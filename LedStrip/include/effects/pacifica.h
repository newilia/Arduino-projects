...
void pacifica()
{
	...
	EVERY_N_MILLIS(100)
	{
		utils::IrControlValue<float>(BUTT_DOWN, BUTT_UP, g_settings.pacifica.speed, 0.1, 5, 0.3);
	}
}
...
void pacifica_loop()
{
	...
	sLastms = ms;
	auto & speed = g_settings.pacifica.speed;
	uint16_t speedfactor1 = beatsin16(3, 179 * speed, 269 * speed);
	uint16_t speedfactor2 = beatsin16(4, 179 * speed, 269 * speed);
	...
	fill_solid(g_leds, LEDS_COUNT, CRGB(2, 6, 10));
	...
	pacifica_one_layer(palette3, sCIStart4, 5 * 256, beatsin8(8, 10, 28), beat16(601));
